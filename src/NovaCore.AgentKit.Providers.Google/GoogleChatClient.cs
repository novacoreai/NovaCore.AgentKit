...
public class GoogleChatClient : ILlmClient
{
    ...
    private List<object>? BuildTools(LlmOptions? options)
    {
        ...
        _logger?.LogDebug("Building {Count} tools for Google", options.Tools.Count);
        ...
        foreach (var kvp in options.Tools)
        {
            ...
            try
            {
                ...
                _logger?.LogDebug("Built tool {Tool} for Google", tool.Name);
            }
            catch (Exception ex)
            {
