...
/// <summary>
/// Adapts MCP tool to ITool interface
/// </summary>
public class McpToolAdapter : ITool
{
    ...
    {
        // Parse arguments
        var parsedArgs = JsonSerializer.Deserialize<JsonElement>(argsJson);
        ...
        if (parsedArgs.ValueKind == JsonValueKind.Object)
        {
            ...
            {
                // Only unwrap if "args" is the sole property or if it contains the actual parameters
                var propertyCount = 0;
                foreach (var _ in parsedArgs.EnumerateObject()) propertyCount++;
                
                if (propertyCount == 1)
                {
                    // Single "args" property - unwrap it
                    args = JsonSerializer.Deserialize<Dictionary<string, object?>>(argsProperty.GetRawText())
                        ?? new Dictionary<string, object?>();
                }
                else
                {
                    // Multiple properties, use as-is
                    args = JsonSerializer.Deserialize<Dictionary<string, object?>>(argsJson)
                        ?? new Dictionary<string, object?>();
                }
            }
            // Check if arguments are wrapped in "parameters" key (alternative pattern)
            ...
            {
                // No wrapping detected, use as-is
                args = JsonSerializer.Deserialize<Dictionary<string, object?>>(argsJson)
                    ?? new Dictionary<string, object?>();
            }
        }
        ...
        // MCP results typically have a 'content' array with text/image content
        var resultStr = result.Data?.GetRawText() ?? "{}";
        
        // If the result is empty or just {}, return a descriptive message
